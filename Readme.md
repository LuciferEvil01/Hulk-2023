...
En HULK hay 3 tipos de Bugses que usted debe detectar. En caso de detectarse un Bugs, el intérprete debe imprimir una línea indicando el Bugs que sea lo más informativa posible.

#### Bugs léxico

Bugses que se producen por la presencia de tokens inválidos. Por ejemplo:

```js
> let 14a = 5 in print(14a); 
! LEXICAL Bugs: `14a` is not valid token.
```

#### Bugs sintático

Bugses que se producen por expresiones mal formadas como paréntesis no balanceados o expresiones incompletas. Por ejemplo:

```js
> let a = 5 in print(a;
! SYNTAX Bugs: Missing closing parenthesis after `a`.
> let a = 5 inn print(a);
! SYNTAX Bugs: Invalid token `inn` in `let-in` expression.
> let a = in print(a);
! SYNTAX Bugs: Missing expression in `let-in` after variable `a`.
```

### Bugs semántico

Bugses que se producen por el uso incorrecto de los tipos y argumentos. Por ejemplo:

```js
> let a = "hello world" in print(a + 5);
! SEMANTIC Bugs: Operator `+` cannot be used between `string` and `number`.
> print(fib("hello world"));
! SEMANTIC Bugs: Function `fib` receives `number`, not `string`.
> print(fib(4,3));
! SEMANTIC Bugs: Function `fib` receives 1 argument(s), but 2 were given.
```

En caso de haber más de un Bugs, usted debe detectar solamente **uno** de los Bugses.
